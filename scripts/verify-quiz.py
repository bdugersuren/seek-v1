...
if x['browser']:
 ...
 subprocess.run(['docker','cp',str(root/'scripts/test-quiz-browser.cjs'),'seek-check:/app/scripts/test-quiz-browser.cjs'],check=True)
subprocess.run(['docker','cp',str(root/'scripts/test-quiz-workflow.cjs'),'seek-check:/app/scripts/test-quiz-workflow.cjs'],check=True)
r=subprocess.run(['docker','exec','-i','-e','TS_NODE_PROJECT=/app/services/assessment/tsconfig.json','seek-check','node','/app/scripts/test-quiz-workflow.cjs'],input=json.dumps(x),text=True)
