...
EVIDENCE = ROOT / '.production/evidence/quiz-workflow'
IMAGES = {
    'assessment': ('SEEK_ASSESSMENT_IMAGE', 'seek-backend:quiz-workflow-20260913'),
    'gateway': ('SEEK_GATEWAY_IMAGE', 'seek-backend:quiz-workflow-20260913'),
    'portal-web': ('SEEK_PORTAL_IMAGE', 'seek-portal-web:quiz-workflow-20260913'),
}
...
def main():
    ...
    for database, table in [('assessment_db','quiz_template')]:
        ...
        os.chmod(path,0o600)
        temporary='quiz_check_'+database+'_'+stamp.replace('-','_')
        ...
        try:
            ...
            assert temporary.startswith('quiz_check_')
    ...
    try:
        ...
        table=subprocess.check_output(['docker','exec','seek-postgres-1','psql','-U','seek_admin','-d','assessment_db','-Atc',"SELECT to_regclass('quiz_mutation_request') IS NOT NULL"],text=True).strip()
        active='0'
        if table=='t':
            active=subprocess.check_output(['docker','exec','seek-postgres-1','psql','-U','seek_admin','-d','assessment_db','-Atc','SELECT count(*) FROM quiz_mutation_request'],text=True).strip()
        if active!='0':
            raise RuntimeError('New Quiz workflow mutations exist; retain compatible backend and repair forward. Do not silently roll back workflow semantics.')
