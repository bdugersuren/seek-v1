...
export function QuestionTypePreview({
  ...
  if (question.type === "MATRIX") {
    const columns: Array<{ id: string; label: string }> =
      question.scoringConfig?.matrixColumns || [];
    if (!columns.length)
      return <p role="status">Матрицын баганууд тохируулагдаагүй байна.</p>;
    return (
      <div className="overflow-x-auto rounded border">
        <table className="w-full text-sm">
          <caption className="sr-only">Матрицын зөв хариулт ба оноо</caption>
          <thead>
            <tr>
              ...
              {columns.map((c) => (
                <th scope="col" className="p-3" key={c.id}>
                  {c.label}
                </th>
              ))}
            ...
            {question.options.map((row, i) => (
              <tr key={row.id} className="border-t">
                <th scope="row" className="p-3 text-left font-normal">
                  <RichTextPreview value={row.value} />
                </th>
                {columns.map((c) => (
                  <td key={c.id} className="p-3 text-center">
                    <span
                      aria-label={
                        row.matchValue === c.id ? "Зөв хариулт" : "Сонгоогүй"
                      }
                    >
                      {row.matchValue === c.id ? "●" : "○"}
                    </span>
  ...
  if (question.type === "ESSAY") {
    const rubrics =
      question.rubric && Array.isArray(question.rubric) ? question.rubric : [];
          ...
          <div className="rounded-seek-md border border-slate-200 bg-slate-50/50 p-seek-4">
            ...
            <div className="space-y-2">
              {rubrics.map((r: any, idx: number) => (
                <div
                  key={r.id || idx}
                  className="flex items-center justify-between bg-white border border-slate-200 rounded-seek-md p-seek-3 text-xs"
                >
                  <div>
        ...
        {question.type === "NUMERIC" ? (
          ...
          <div className="space-y-3">
            {question.options.map((opt, idx) => {
              const accepted =
                opt.acceptedValues && opt.acceptedValues.length > 0
                  ? opt.acceptedValues
                  : [
                      {
                        value: opt.value || "-",
              ...
              return (
                <div
                  key={opt.id || idx}
                  className="rounded-seek-md border border-slate-200 border-l-[4px] border-l-teal-500 bg-white p-3 text-xs shadow-seek-xs space-y-2"
                >
                  <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
                    <span className="font-bold text-teal-900">
                      Хоосон зай #{idx + 1} ({`{{blank_${idx + 1}}}`} /{" "}
                      {`[[${idx + 1}]]`})
                    </span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {accepted.map((av: any, aIdx: number) => (
                      <div
                        key={aIdx}
                        className="flex items-center gap-1.5 bg-slate-50 border border-border rounded-seek-md px-2.5 py-1"
                      >
      ...
      <div className="space-y-seek-4">
        <div className="space-y-seek-3">
          <Text variant="muted" className="text-sm font-semibold">
            Харгалзуулах хосуудын зөв тохиргоо:
          </Text>
          <div className="grid gap-seek-3 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              {question.options.map((option, idx) => (
                <div
                  key={`left-${option.id}-${idx}`}
                  className="rounded-seek-md border border-emerald-200 border-l-[4px] border-l-emerald-500 bg-emerald-50/15 overflow-hidden flex shadow-seek-xs"
                >
            ...
            <div className="space-y-2">
              <Text className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                Баруун тал (Хариултууд)
              </Text>
              {(
                question.scoringConfig?.rightOptions ||
                (question.contentJson as any)?.scoringConfig?.rightOptions ||
                (question.contentJson as any)?.payload?.scoringConfig
                  ?.rightOptions ||
                question.options
                  .map((o) => ({ id: o.id, value: o.matchValue }))
                  .filter((o) => o.value)
              ).map((valObj: any, idx: number) => (
                <div
                  key={`right-${valObj.id || idx}`}
                  className="rounded-seek-md border border-indigo-200 border-l-[4px] border-l-indigo-500 bg-indigo-50/15 overflow-hidden flex shadow-seek-xs"
                >
        ...
        {!isCombinationScoring && (
          <ul className="text-sm">
            {question.options.map((row, i) => {
              const right = (
                question.scoringConfig?.rightOptions || []
              ).findIndex((r: any) => r.id === row.matchValue);
              return (
                <li key={row.id}>
                  L{i + 1} →{" "}
                  {right < 0 ? "Харгалзаа сонгоогүй" : `R${right + 1}`} ·{" "}
                  {row.score} оноо
                </li>
              );
            })}
  ...
  if (question.type === "ORDERING") {
    return (
      <div className="space-y-seek-4">
        <div className="space-y-seek-3">
          <Text variant="muted" className="text-sm font-semibold">
            Алхмуудын зөв дараалал:
          </Text>
          <div className="space-y-2">
            {question.options.map((option, idx) => (
              <div
                key={option.id || idx}
                className="rounded-seek-md border border-amber-200 border-l-[4px] border-l-amber-500 bg-amber-50/15 overflow-hidden flex shadow-seek-xs"
              >
        ...
        {question.options.map((option, index) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`w-8 h-8 rounded-seek-md flex items-center justify-center font-bold text-sm shrink-0 select-none bg-white border ${indicatorBgClass}`}
              >
      ...
      <div className="border border-slate-100 bg-white rounded-seek-lg divide-y divide-slate-100 shadow-seek-xs overflow-hidden">
        {combinations.map((combo: any, idx: number) => {
          ...
          if (question.type === "MULTIPLE_CHOICE") {
            ...
            const rightItems =
              question.scoringConfig?.rightOptions ||
              (question as any).contentJson?.scoringConfig?.rightOptions ||
            ...
            const pairs = (combo.ids || combo.answers || [])
              .map((pairStr: string, pairIdx: number) => {
                ...
                const leftLabel = leftOpt ? leftOpt.label : "Устсан мөр";
                ...
                let rightIdx = rightItems.findIndex((r: any) => {
                  ...
                  return (
                    cleanOptRId === cleanRId ||
                    cleanRId.endsWith(cleanOptRId) ||
                    cleanOptRId.endsWith(cleanRId)
                  );
                });
                ...
                const rightLabel =
                  rightIdx !== -1 ? `R${rightIdx + 1}` : "Устсан хариулт";
            ...
            const orderLabels = (combo.ids || combo.answers || [])
              .map((id: string) => {
                ...
                return opt ? opt.label : "Устсан сонголт";
              })
              .filter(Boolean)
              .join(" ➔ ");
          ...
          if (
            ...
            (combo.score === question.defaultMaxScore &&
              question.type === "MULTIPLE_CHOICE" &&
              (combo.ids || combo.answers || []).length === 1)
          ) {
            comboDescription = comboDescription.replace(
              /selected|matched|ordered|filled/,
              "only (correct)",
            );
          }
